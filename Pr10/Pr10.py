...
def c():
    if combo.get() == "+":
        ...
        label1.configure(text=int(spin.get()) / int(spin1.get()))
def c1():
    if selected.get() == 0: messagebox.showinfo("Окно выбора", "Вы выбрали первый вариант")
    if selected.get() == 1: messagebox.showinfo("Окно выбора", "Вы выбрали второй вариант")
    if selected.get() == 2: messagebox.showinfo("Окно выбора", "Вы выбрали третий вариант")
def c2():
...
def c3():
    ...
    file1.close()
window = Tk()
window.title("Ткаченко Даниил Алексеевич")
...
btn = Button(tab1, text="=", command=c)
...
btn1 = Button(tab2, text="Тык", command=c1)
...
btn2 = Button(tab3, text="Загрузить", command=c2)
btn2.grid(row=1, column=0, pady=10)
btn3 = Button(tab3, text="Сохранить", command=c3)
